...
describe('<StaffManagement />', () => {
  it('renders the staff list correctly without crashing', () => {
    ...
    it('should display the staff in the correct grid layout', () => {
        ...
        const availableStaffContainer = screen.getByText('Available Staff').nextSibling as HTMLElement;
        const busyStaffContainer = screen.getByText('Busy Staff').nextSibling as HTMLElement;
        ...
        if (availableStaffContainer && availableStaffContainer.tagName === 'DIV') {
          expect(availableStaffContainer.className).toContain('grid grid-cols-2 gap-4');
        } else {
          throw new Error('Available staff container not found or incorrect element type');
        }
