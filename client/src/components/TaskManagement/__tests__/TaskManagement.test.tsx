...
describe('<TaskManagement />', () => {
  ...
  it('sorts tasks by status correctly', () => {
    ...
    const notStartedTasks = testTaskData.filter(
      (task) => task.status === 'Not Started',
    );
    const inProgressTasks = testTaskData.filter(
      (task) => task.status === 'In Progress',
    );
    const blockedTasks = testTaskData.filter(
      (task) => task.status === 'Blocked',
    );
    const completedTasks = testTaskData.filter(
      (task) => task.status === 'Completed',
    );

    expect(screen.getAllByText('Not Started').length).toBe(
      notStartedTasks.length + 1,
    );
    expect(screen.getAllByText('In Progress').length).toBe(
      inProgressTasks.length + 1,
    );
    expect(screen.getAllByText('Blocked').length).toBe(blockedTasks.length + 1);
    expect(screen.getAllByText('Completed').length).toBe(
  ...
  it('displays tasks under correct headings', () => {
    ...
    notStartedTasks.forEach((task) => {
      expect(screen.getByText(task.type).closest('div')).toHaveTextContent(
        'Not Started',
      );
    });
  ...
  it('renders no tasks if tasks array is empty', () => {
    render(
      <QueryClientProvider client={queryClient}>
        <TaskManagement tasks={[]} staff={testStaffData} gameId={gameId} />
      </QueryClientProvider>,
    );
